...
def refinar(my_list):  # Remueve los términos don't care de una lista dada y retorna la lista refinada
    res = []
    for i in my_list:
        res.append(i)
    return res
...
def BuscarVariables(
        x):  # Función para encontrar las variables en los términos. Por ejemplo, el mintérmino --01 tiene a C' y D como variables
    var_list = []
...
def buscaMinterminos(
        a):  # Función para encontrar a los mintérminos mezclados. Por ejemplo, 10-1 son obtenidos al combinar 9(1001) y 11(1011)
    gaps = a.count('-')
    if gaps == 0:
        return [str(int(a, 2))]
...
mt = [int(i) for i in input("Ingrese los minitérminos ").strip().split()]
mt.sort()
minterminos = mt
minterminos.sort()
...
print("\n\n\n\nNúmero de Gpo.\tMinitérminos\t\tExpresión en BCD\n%s" % ('=' * 60))
...
while True:
        ...
        for j in tmp[l[i]]:  # Iteración a través del grupo de elementos actual
            for k in tmp[l[i + 1]]:  # Iteración a través del siguiente grupo de elementos
                ...
                if res[0]:  # Si los mintérminos difieren solamente en un bit
                    try:
                        grupos[m].append(j[:res[1]] + '-' + j[res[1] + 1:]) if j[:res[1]] + '-' + j[res[1] + 1:] not in \
                                                                               grupos[
                                                                                   m] else None  # Imprimimos un guión '-' en el bit que cambia y lo agregamos al grupo correspondiente
                    except KeyError:
                        grupos[m] = [j[:res[1]] + '-' + j[res[
                                                              1] + 1:]]  # Si el grupo no existe, crearemos un grupo al principio y pondremos un guión '-' en el cambio de bi, además de agregarlo a un nuevo grupo
                    debo_parar = False
    ...
    print("Elementos desmarcados(Implicantes Primos) de la tabla:", None if len(desmarcados_local) == 0 else ', '.join(
        desmarcados_local))  # Imprimimos los implicantes promos en la tabla actual
    if debo_parar:  # Si los mintérminos no pueden ser combinados
        print("\n\nTodos Implicantes Primos: ",
              None if len(all_pi) == 0 else ', '.join(all_pi))  # Imprimimos todos los implicantes primos
        break
    ...
    for i in sorted(grupos.keys()):
        ...
        for j in grupos[i]:
            print("\t\t%-24s%s" % (
            ','.join(buscaMinterminos(j)), j))  # Imprimimos los mintérminos y su representación binaria.
        print('-' * 60)
...
for i in all_pi:
    ...
    for j in refinar(minterminos_mezclados):
...
if (len(
        chart) == 0):  # Si los imintérminos premanecen después de remover los Implicantes Primos de las columnas relacionadas
    resultado_final = [BuscarVariables(i) for i in
                       IPE]  # Resultado Final solamente con los Implicantes Primos Escenciales
    ...
    resultado_final.extend(
        BuscarVariables(i) for i in IPE)  # Agregamos los Implicantes Primos Escenciales en la solución
